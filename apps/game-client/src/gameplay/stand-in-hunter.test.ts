...
const CFG: StandInHunterConfig = { speed: 120, footprintRadius: 16, wanderTurnRate: 0.8, combatSpeedMultiplier: 1.0 };
...
describe('stand-in hunter', () => {
  ...
  it('chases toward the player while in combat', () => {
    const s = createStandInHunter({ x: 300, y: 300 });
    const player = { x: 260, y: 300 }; // player to the WEST
    const next = stepStandInHunter(s, player, true, 0.1, world, CFG);
    expect(next.position.x).toBeLessThan(s.position.x); // moved WEST, toward player
  });
